...
def test_usepl0():
    ...
    try:
        launch_testprogram(connection_timeout_sec=2.0)
...
def test_usepl1_nocontrol():
    ...
    CHECK(
        status != 0 and "Control" in answer,
        "CLI to get some poetry failed as expected (no control)",
    )
...
def test_usepl1_noevent():
    ...
    status, answer = program_cli("get_poetry")
    CHECK(
        status == 0 and answer == "To bug, or not to bug,that is the question",
        "CLI to get some poetry called successfully",
        status,
        answer,
    )
    CHECK(not data_collect_events(timeout_sec=2.0), "No event is received")
    process_stop()
...
def test_usepl1_noassert():
    ...
    try:
        ...
        )  # Wait first freeze to get all CLIs
        CHECK(True, "Connection established")
    except ConnectionError:
        CHECK(False, "No connection")
...
def test_usepl1_saturated_request_buffer():
    ...
    CHECK(
        status == 0,
        "CLI with small command request size called successfully",
        status,
        answer,
    )
...
def test_usepl1_saturated_response_buffer():
    ...
    status, answer = program_cli(
        "test::parametersDft third=[[hello]]"
    )  # The answer with payload is just less than 110 bytes
    CHECK(
        status == 0,
        "CLI with small command reponse size called successfully",
        status,
        answer,
    )
    status, answer = program_cli(
        "test::parametersDft third=[[hello, what is the weather today?]]"
    )  # The answer with payload exceeds the 110 bytes
    events = data_collect_events(timeout_sec=2.0)
    CHECK(
        status != 0
        and "CLI response buffer is full" in answer
        and process_is_running(),
...
def test_usepl1_saturated_cli_qty():
    ...
    CHECK(
        not process_is_running() and not events,
        "No event received because an assert failed (max CLI exceeded)",
        process_is_running(),
        len(events),
    )
    process_stop()
...
def test_usepl1_saturated_cli_param_qty():
    ...
    CHECK(
        not process_is_running() and not events,
        "No event received because an assert failed (max CLI param exceeded)",
        process_is_running(),
        len(events),
    )
    process_stop()
...
def test_short_hash_string():
    ...
    CHECK(data_collect_events(timeout_sec=2.0), "Fruit related events are received")
    process_stop()
...
def test_32bits_arch():
    ...
    build_target(
        "testprogram",
        "USE_PL=1 PL_IMPL_STACKTRACE=0 PL_SHORT_STRING_HASH=1",
        compilation_flags=["-m32"],
    )
...
def test_external_string():
    ...
    # With decoding
    set_external_strings(
        lkup={hash_string("Add fruit"): "Add fruit", hash_string("CRASH"): "CRASH"}
    )
    try:
        launch_testprogram()
        CHECK(True, "Connection established")
